...
class MultiplePeaksModel2:
    ...
    def obj_grad(self, phenome):
        phenome = np.array(phenome)
        peak = self.getActivePeak(phenome)
        peaks = copy.copy(self.peaks)
        peaks.remove(peak)
        
        distance = self.dist(phenome, peak)
        differenceVector = phenome - np.array(peak)
        
        try:
            return peak.height / (1.0 + math.pow(distance, peak.shape) / peak.radius) ** 2.0 * \
                peak.shape / peak.radius * math.pow(distance, peak.shape-2.0) * \
                np.dot(differenceVector, peak.D)
        except ValueError: # in case of non-differentiable points
            return [0.0] * self.numberVariables
    ...
    def getActivePeak2(self, phenome):
        g = self.g
...
def evaluateProblemGrad(position, npeaks, dimension, topology, randomSeed, rotated, peakShape):
    global currentProblem
    initProblem(npeaks, dimension, topology, randomSeed, rotated, peakShape)
    return currentProblem.obj_grad(position)
...
def createProblem(npeaks, dimension, topology, randomSeed, rotated, peakShape):
    # get the original random state
    state1 = np.random.get_state()
    state2 = random.getstate()
    ...
    newProblem = MultiplePeaksModel2.createInstanceWithExactNumberOfOptima(npeaks, \
        dimension, topology, shapeHeightCorrelation = 0, rotatedPeaks = rotated, \
        peakShape = peakShape)
...
if __name__ == "__main__":
    ...
    a = np.array([ 0.4359949 ,  0.02592623])  
    P1 = initProblem(1, 2, 'random', 1, False, 'sphere')
    
    print P1.objectiveFunction(a)
    
    
    
#    P2 = initProblem(2, 2, 'random', 50, False, 'sphere')
#    print P1.getLocalOptima()
#    print P2.getLocalOptima()
#    print P1.objectiveFunction([2, 2]), P2.objectiveFunction([2, 2])
#    
